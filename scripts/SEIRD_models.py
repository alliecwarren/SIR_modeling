...
#region Disease Parameters and Vaccination Data
disease_params = {'varicella': {'incubation_period': [15, 10, 21], # https://www.cdc.gov/pinkbook/hcp/table-of-contents/chapter-22-varicella.html
                  ...
                  'rubella': {'incubation_period': [14, 12, 23], # https://www.cdc.gov/pinkbook/hcp/table-of-contents/chapter-20-rubella.html
                                'infection_period': [41], # https://www.researchgate.net/publication/351105137_THE_DYNAMICS_OF_RUBELLA_VIRUS_WITH_TWO-DOSE_VACCINATION_STRATEGY
                                ...
                                'fatality_rate': [.00029]}, # https://www.researchgate.net/publication/351105137_THE_DYNAMICS_OF_RUBELLA_VIRUS_WITH_TWO-DOSE_VACCINATION_STRATEGY
...
diseases = ['measles', 'pertussis', 'rubella', 'varicella']
vacc_subset = pd.read_csv(base_folder + "WA_school_vaccination_combined.csv")
#endregion

# region SEIRD functions
...
def stochastic_seird_model(initial_conditions, t_span, t_step, 
                          beta, sigma, gamma, mu):
    """
    Stochastic SEIRD model with transitions drawn from Poisson distributions
    
    Args:
        initial_conditions: tuple (initS, initE, initI, initR, initD)
        t_span: tuple (t_start, t_end)
        t_step: time step for simulation
        beta, sigma, gamma, mu: transition parameters for SEIRD
    """
    ...
    for i in range(1, n_steps):
       ...
       # draw transitions from poisson distribution to incorporate stochasticity
        dSdt = beta*S*I/N
        dSdt = max(0, dSdt)
        dSdt = poisson(dSdt)*t_step

        e_component = max(0, sigma*E)
        e_component = poisson(e_component)*t_step
        dEdt = dSdt - e_component

        gamma_component = max(0, gamma*I)
        gamma_component = poisson(gamma_component)*t_step

        mu_component = max(0, mu*I)
        mu_component = poisson(mu_component)*t_step

        dIdt = e_component - gamma_component - mu_component
        dRdt = gamma_component
        dDdt = mu_component
...
def run_simulation(initial_conditions = (200, 1, 1, 0, 0), t_max =100, beta_mean=3, sigma_mean=(1/10.5), gamma_mean=(1/5), mu_mean=(1.5/1000)):
    '''
    Run simulation to calculate infectious counts with values drawn from poisson distribution
    initial_conditions: susceptible count, exposed count, infected count, recovered count, deceased count
    t_max: number of days of simulation
    beta_mean, sigma_mean, gamma_mane, mu_mean: transition parameters for SEIRD model, default values approx. for measles

    returns time list and infectious counts
    '''
    t_span = (0, t_max)
...
def calc_potential_infectious(num_scenarios, initial_conditions, t_max, beta_mean, sigma_mean, gamma_mean, mu_mean):
    '''
    Run simulation for X scenrarios to calculate infectious counts with values drawn from poisson distribution
...
def plot_scenarios(num_scenarios = 10, initial_conditions = (200, 1, 1, 0, 0), t_max =100, beta_mean=3, sigma_mean=(1/10.5), gamma_mean=(1/5), mu_mean=(1.5/1000)):
    '''
    Create plot of infectious counts using stochastic simulation of SEIRD model
...
def create_plot(initN, initE = 1, initI = 1, initR = 0, initD = 0, R0 = 15, sigma = (1/10.5), gamma = (1/5), mu = (1.5/1000), days = 200):
    """
...
location_tab = ui.nav_panel(
    ...
    ui.layout_sidebar(
        ui.sidebar(
            ui.input_select("disease", "Disease:", choices=diseases),
            ui.h3("Select School"),
            ui.input_select("county", "County:", choices=sorted(vacc_subset['County'].unique())),
            ui.output_ui("district_ui"),
            ui.output_ui("school_ui"),
            ui.input_slider("number_of_days", "Simulation Days:", 
                          min=20, max=400, value=100, step=10),
        ),
...
def server(input, output, session):
    ...
    # Dynamic UI elements for county selection based on disease
    @output
    @render.ui
    def county_choices():
        return ui.input_select("county", "County:", choices=sorted(vacc_subset['County'].unique()))
    ...
    @reactive.Calc
    @reactive.event(input.county, input.district, input.school, input.disease)
    def get_selected_school_info():
        ...
        school_data = school_data[school_data['disease'] == input.disease()]
    ...
    def selected_school_info():
        ...
        cur_params = disease_params[disease]
        incub = cur_params['incubation_period'][0]
        infect = cur_params['infection_period'][0]
        r0 = cur_params['r0'][0]
        fatal = cur_params['fatality_rate'][0]
        vacc_eff = cur_params['vaccine_efficacy'][0]
        ...
        vacc_failure_rate = 1-(vacc_eff/100)
        ...
        return (f"County: {school_data.iloc[0]['County']}\n"
                ...
                f"Basic Reproductive Number: {r0}\n"
                f"Incubation Period (days): {incub}\n"
                f"Infection Period (days): {infect}\n"
                f"Fatality Rate: {fatal}\n"
                f"Vaccine Efficacy: {vacc_eff}%\n")
    ...
    def location_plot():
        ...
        if school_data is None:
            ...
            fig = create_plot(initN=totalN, initR = vaccine_protected, R0 = r0, sigma = 1/incub, gamma = 1/infect, mu = fatal, days = input.number_of_days())
            plot_html = to_html(fig, include_plotlyjs="cdn", full_html=True)
    ...
    # Plot stochastic model based on selected parameters
    @output
    @render.ui
    def infected_plot():
